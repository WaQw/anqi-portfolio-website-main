...
type ProjectProps = (typeof data)[number];
...
export default function Project({ title, description, tags, imageUrl, link }: ProjectProps) {
    ...
    return (
        ...
        style={{
            ...
            <section 
                className='max-w-[42rem] border border-black/5 rounded-lg overflow-hidden sm:pr-8 relative sm:h-[20rem] even:pl-8 hover:bg-gray-200 transition group-even:pl-8'
            >
               <a href={link}>
               <div className='pt-4 pb-7 px-5 sm:pl-10 sm:pr-2 sm:pt-10 sm:max-w-[50%] flex flex-col h-full group-even:ml-[18rem]'>
                    <h3 className='text-2l font-semibold'>{title}</h3>
                    <p className='text-sm mt-2 leading-relaxed text-gray-700 text-left'>{description}</p>
